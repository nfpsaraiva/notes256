...
contract ProofContract is ERC721, Ownable {
    ...
    event ProofCreated(uint256 indexed proofId, address indexed issuer, string ipfsHash, string proofType, string description);
    event NFTIssued(uint256 indexed proofId, address indexed recipient, uint256 tokenId);
     ...
     */
    function createProof(string memory _ipfsHash, string memory _proofType, string memory _description) external {
        proofCounter++;
        proofs[proofCounter] = Proof(_ipfsHash, _proofType, _description, msg.sender, block.timestamp);
        emit ProofCreated(proofCounter, msg.sender, _ipfsHash, _proofType, _description);
